...
const processEvent = async (
  ...
  const processTemplates = async (
    ...
    const downloadTemplates = async (
      ...
      const getBearerToken = async () => {
        ...
        app.log.debug(
          `Generating bearer token valid for ${minutesValid} minute.`
        );
      ...
      const fetched = await fetch(release.zipball_url, {
  ...
  const commitFiles = async (
    ...
    const getOrCreateNewBranch = async () => {
      try {
        ...
        app.log.debug(
          `Failed to create a new branch with ref: '${fullBranchName}'.`
        );
        app.log.debug(
          `Fetching existing branch with ref: '${fullBranchName}'.`
        );
        const foundBranch = (
        ...
        app.log.debug(`Found new branch with ref: '${newBranch.ref}'.`);
        return foundBranch;
      }
    };
    ...
    const updateBranch = async () => {
      app.log.debug(
        `Setting new branch ref '${newBranch.ref}' to commit '${newCommit.data.sha}'.`
      );
    ...
    const createOrUpdatePullRequest = async () => {
      ...
      if (automerge) {
        app.log.debug(
          `Attempting automerge of PR #${pullRequest.data.number}.`
        );
    ...
    const currentCommit = await context.octokit.git.getCommit({
      ...repository,
      commit_sha: newBranch.object.sha,
    });
  ...
  if (monitorAllBranches || branchesToProcess.test(payload.ref)) {
    ...
    try {
      ...
      app.log.info(
        `Processing changes made to ${repository.owner}/${repository.repo} in ${payload.after}.`
      );
      ...
      if (filesChanged.includes(configFileName)) {
        const parsed: RepositoryConfiguration =
          await determineConfigurationChanges(configFileName, repository);

        const processed = await processTemplates(parsed, context);

        const shouldAutomerge = parsed.automerge ?? false;
        const pullRequestNumber = await commitFiles(
          repository,
          processed,
          shouldAutomerge,
          context
        ...
        app.log.info(
          `See: https://github.com/${repository.owner}/${repository.repo}/pull/${pullRequestNumber}`
        );
      }
    } catch (e: any) {
