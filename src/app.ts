...
const processPushEvent = (branchesToProcess: RegExp) => async (payload: PushEvent, context: Context<'push'>, app: Probot) => {
  ...
  try {
    ...
    if (filesChanged.includes(configFileName)) {
      const parsed = await determineConfigurationChanges(app, context)(
        configFileName,
        repository,
        payload.after,
      );
      ...
      app.log.info(
        `Committed templates to '${repository.owner}/${repository.repo}' in #${pullRequestNumber}`,
      );
      app.log.info(
...
export = (app: Probot) => {
  ...
  app.on('push', async (context: Context<'push'>) => {
    await processPushEvent(branchesToProcess)(context.payload as any, context, app);
  });
};
