...
describe('Template rendering', () => {
  test('should render basic json and toml template', async () => {
    ...
    //then
    expect(renderedTemplates).toBeTruthy()
  ...
  test('should render relative path templates with expected prefix', async () => {
    ...
    expect(renderedTemplates?.templates?.length).toEqual(2)
    expect(renderedTemplates?.templates[0].contents).toEqual('{"owner": "pleo", "repo": "expected-app-name"}')
    expect(renderedTemplates?.templates[1].contents).toEqual('owner = "pleo" repo = "expected-app-name"')
    expect(renderedTemplates?.templates[0].sourcePath).toEqual(contents[0].path)
    expect(renderedTemplates?.templates[1].sourcePath).toEqual(contents[1].path)
  ...
  test('should render mixed relative path templates and (source, destination) with expected prefixes', async () => {
    ...
    const renderedTemplates = await renderTemplates(configuration)
    expect(renderedTemplates?.templates.length).toEqual(4)
    expect(renderedTemplates?.templates[0].contents).toEqual('{"owner": "pleo", "repo": "expected-app-name"}')
    expect(renderedTemplates?.templates[1].contents).toEqual('owner = "pleo" repo = "expected-app-name"')
    expect(renderedTemplates?.templates[2].contents).toEqual('{"owner": "pleo", "repo": "expected-app-name"}')
    expect(renderedTemplates?.templates[3].contents).toEqual('owner = "pleo" repo = "expected-app-name"')
    expect(renderedTemplates?.templates[0].sourcePath).toEqual(contents[0].path)
    expect(renderedTemplates?.templates[1].sourcePath).toEqual(contents[1].path)
    expect(renderedTemplates?.templates[2].sourcePath).toEqual(contents[2].path)
    expect(renderedTemplates?.templates[3].sourcePath).toEqual(contents[3].path)
