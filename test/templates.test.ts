...
describe('Template rendering', () => {
  test('should render basic json and toml template', async () => {
    ...
    const { renderFiles } = release(log, mockedOctokit)

    //when
    const renderedTemplates = await renderFiles(configuration)

    //then
    expect(renderedTemplates.files.length).toBe(2)

    //and
    expect(renderedTemplates.files[0].contents).toBe('{"owner": "pleo", "repo": "expected-app-name"}')
    expect(renderedTemplates.files[1].contents).toBe('owner = "pleo" repo = "expected-app-name"')
  })
  ...
  test('should render relative path templates with expected prefix', async () => {
    ...
    const { files: result } = await renderFiles(configuration)
  ...
  test('should add default prepending header for `toml` file when CODEOWNERS defined', async () => {
    ...
    //then
    expect(renderedTemplates.files[0].contents).toBe(
...
repo = "expected-app-name"`,
  ...
  test('should assign global CODEOWNERS when not defined explicitly for file', async () => {
    ...
    //then
    expect(renderedTemplates.files[0].contents).toBe('#OWNER: @pleo-io/global_team\n\nrepo = "expected-app-name"')
  })
  ...
  test('should assign pattern CODEOWNERS when not defined explicitly for file', async () => {
    ...
    //then
    expect(renderedTemplates.files[0].contents).toBe('#OWNER: @pleo-io/team_from_pattern\n\nrepo = "expected-app-name"')
    expect(renderedTemplates.files[1].contents).toBe(
